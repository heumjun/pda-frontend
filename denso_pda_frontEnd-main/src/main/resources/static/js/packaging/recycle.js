...
const recycle = function(){
	...
	let st12LineCode = input.comboBox('#st12LineCode', getComboLineList(), 'lineCode','lineNm');
	let st12EquipCode = input.comboBox('#st12EquipCode', getComboEquipCodeList(), 'cm07Code','cm07Name');
    /**
     * 그리드 초기화
     */
    const gridInit = () => {
        ...
        let columnsDefinition = [
			...
			{binding:'st12Gbn'		,header:'구분'		,width:90	,align:'center'		,dataType:'String'	,visible:false},
        ];
        ...
        grid._flexCv.getError = (item,prop)=>{
			...
			let sameCode = grid._flexCv.sourceCollection.filter((c) =>
							( c.st12Company == item.st12Company && c.st12Factory == item.st12Factory && c.st02Code == item.st12Code && c.st12Qr == item.st12Qr));
	...
	const barcodeSearch = async(barcode) => {
	    ...
	    try {
	        let {recycleInfo} = await ajax.getAjax(params, true);

			if ( recycleInfo != null ) {
				 
				let addRow = grid._flexCv.addNew();
				
				addRow.st12LotSeq = recycleInfo.st12LotSeq;
				addRow.st12No = recycleInfo.st12No;
				addRow.st12Code = recycleInfo.st12Code;
				addRow.st12Name = recycleInfo.st12Name;
				addRow.st12Pcc = recycleInfo.st12Pcc;
				addRow.st12Qty = 1;
				addRow.st12Qr = barcode;
	...
	const saveRecycle = () => {
		...
		confirm("재투입을 등록하시겠습니까?", "재투입 이력이 등록됩니다.", consts.MSGBOX.QUESTION, () => {

			let params = {
                uri: `recycle/recycle`,
                insertList: insertList,
				st12LineCode: st12LineCode.selectedValue,
				st12EquipCode: st12EquipCode.selectedValue
            };
		...
		onComplete: function(barcode, qty) {
			...
			grid._flexGrid.rows.some((row, index, array) => {
				if (!wijmo.isUndefined(row.dataItem) && !wijmo.isNullOrWhiteSpace(row.dataItem)) {
					if(row.dataItem.st12Qr == barcode) {
