...
const returnReg = function(){
    ...
    const gridInit = ()=>{
        ...
        let columnsDefinition = [
            ...
            {binding:'itemNm'	,header:'품목명'	,width:150	,dataType:'String'	,align:'left'	,maxLength:50},
            {binding:'qrCode'	,header:'QR코드'	,width:70	,dataType:'String'	,align:'center'	,maxLength:6,},
            {binding:'itemQty'	,header:'수량'	,width:130	,dataType:'Number'	,editor:numberInput	,isRequired:true},
        ...
        grid.disableReadOnlyForAutoRows(['itemNm','qrCode','storage','area']);
        //대문자로 변경하고싶은 컬럼
        grid.toUpperCase(['itemNm','qrCode','storage','area']);
        ...
        grid._flexCv.getError = (item,prop)=>{
            ...
            switch (prop) {
                case 'itemQty':
                    if(wijmo.isNullOrWhiteSpace(item.itemQty)) return '[수량]를 입력하세요.';
                    break;
                case 'qrCode':
