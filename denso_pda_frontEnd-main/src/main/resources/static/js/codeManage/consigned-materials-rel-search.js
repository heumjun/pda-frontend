...
const returnReg = function(){
    ...
    const gridInit = ()=>{
        ...
        let columnsDefinition = [
			...
			{binding:'mf13Dat'		,header:'요청일자'			,width:110	,dataType:'String'	,align:'center'	,isReadOnly: true},
	...
	grid._flexGrid.addEventListener(grid._flexGrid.hostElement,consts.JQUERYEVENT.DBLCLICK,(e)=>{
	    ...
	    if(ht.panel==grid._flexGrid.cells){    //그리드 셀에 더블클릭했을때
	        if(grid._flexGrid.getColumn(ht.col).binding=='mf15No'){  // aaaa 컬럼 더블클릭했을경우
				...
				form.append($('<input/>', {type: 'hidden', name: 'view', value:'system/consigned-materials-rel' }));
    ...
    const search = async ()=>{
        ...
        let params = {
            uri: `pda/consigned-materials-rel-search`
        }
