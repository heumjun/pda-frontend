...
//Check함수
export const isEmpty = function(checkVal) {
	if(checkVal == undefined || checkVal == null) return true;
	
	if(typeof checkVal != 'object') {
		 if($.trim(checkVal) == '') return true;
		else return false;	
	} else {
		if(Array.isArray(checkVal) && checkVal.length > 0) return false;
		else if(Array.isArray(checkVal) && checkVal.length == 0) return true;
	}
...
$(()=>{
    ...
    let title = $('#data-params').data('params').title;
    $('#title').text(title);
    if(!wijmo.isUndefined($('#menuTitle').get(0))){
        // 타이틀 표시
